...
function makeTask(taskObject) {
    ...
    if (taskObject.isCompleted) {
        ...
        task_shelf.append(undoButton, trashButton);
    } else {
        ...
        task_shelf.append(checkButton, trashButton);
    }
...
function addTask() {
    ...
    const taskObject = generateTaskObject(generatedID, inputTask, inputDate, inputDescription, false);
...
function addTaskToCompleted(titleId) {
    ...
    document.dispatchEvent(new Event(RENDER_EVENT));
    alert('Anda telah menyelesaikan tugas, Tugas akan dialihkan pada daftar lain. Tekan OK untuk melanjutkan');
    saveData();
}
...
function removeTaskFromCompleted(titleId) {
    ...
    var removeTask = confirm("Apakah kamu yakin akan Menghapus Tugas?");
        if (removeTask) {
...
document.addEventListener(RENDER_EVENT, function () {
    const uncompletedTASKList = document.getElementById('incompleteCreatetaskfList');
